...
from .unet import Preprocess, Postprocess
...
class AinrUnetTransformer(torch.nn.Module):
    ...
    def forward(self, present, previous):
        ...
        result = self.postprocess(preprocess[:, :4],
                                  preprocess[:, 4:],
                                  net_out[:, :4] * std_val + mean_val,
                                  net_out[:, 4].unsqueeze(dim=1))
...
if __name__ == "__main__":
    ...
    model = AinrUnetTransformer(unet_arc_parameters)
    result = model(*list(dummy_input.values()))
    summary(model,
            input_data=(list(dummy_input.values())),
            verbose=2)

    onnx_file = './onnx/model_ainr_unet_transformer.onnx'
    ...
    onnx_file = './onnx/model_ainr_unet_transformer_backbone.onnx'
